...
from utils.data_augumentation import Compose, ConvertFromInts, ToAbsoluteCoords, PhotometricDistort, Expand, \
...
def make_datapath_list(rootpath):
    ...
    val_size: float
        訓練データと検証データの分割割合。default=0.3 (train: val = 7: 3)
...
class Anno_xml2list(object):
    ...
    def __call__(self, xml_path, width, height):
        ...
        # class name(物体名の検出)
        name_n_number = xml.find("filename").text.split(".")[0]
        l = name_n_number.split("_")
        name = "_".join(l[:-1])
        # print(f"Name: {name}")
